...
- `mise run verify-vendor` / `update-vendor` / `check-templates` — vendored-asset checksum guard / re-vendor to the pinned `.VERSION` (the script Renovate runs as a postUpgradeTask) / template-comment guard (verify + template also run by prek/CI)
