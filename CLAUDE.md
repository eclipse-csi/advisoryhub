...
- `mise run verify-vendor` / `update-vendor` / `check-templates` / `check-imports` — vendored-asset checksum guard / re-vendor to the pinned `.VERSION` (the script Renovate runs as a postUpgradeTask) / template-comment guard / advisories→workflows import-direction guard (verify, template and imports also run by prek/CI)
...
mise exact-pins only the bootstrap `uv` + `prek` (in `mise.toml`) plus the chart/release binaries (`helm`, `kubeconform`, `git-cliff`, `trivy`) — Renovate tracks those pins via their `# renovate:` annotations (grouped weekly "mise toolchain" PR, review-only); all dev tool versions stay in `uv.lock`, the Python version in `.python-version`. CI runs these same tasks. Raw `uv`/`docker compose` commands remain canonical.
...
Commit stage = file hygiene + `ruff check --fix` + `ruff format`, plus four repo guards: `dev/check_vendored_assets.sh` (htmx, Inter, ALTCHA + the OSV/CSAF/CVE/CVSS JSON schemas — sha256 vs their `*.VERSION` / `SCHEMAS.VERSION` files), `dev/check_template_comments.py` (rejects multi-line `{# #}` — Django's single-line comment renders those as literal text), `dev/check_import_direction.py` (advisories must not import workflows at module level — workflows sits above advisories; the function-local back-edges are frozen to a known allowlist), and zizmor (workflow security audit, fires when `.github/` workflow files change). Push stage adds `mypy`, `makemigrations --check`, `manage.py check`, and `pip-audit` (dependency audit, mirrors CI's security job; needs network). `ty` is manual + advisory (no Django plugin yet), matching CI's `continue-on-error` ty job. Vendored/verbatim assets (`static/htmx.*`, `static/fonts/*.woff2`, `static/fonts/Inter.VERSION`, `static/altcha/*`, `publication/schemas/*.upstream.json`, `publication/schemas/cvss/*.json`, `publication/schemas/SCHEMAS.VERSION`, `docs/assets/css/neoteroi-mkdocs.*`) are excluded from the hygiene hooks.

**Vendored-asset updates** are automated by a *scoped, self-hosted Renovate* workflow (`.github/workflows/renovate.yml` + `.github/renovate.json`) that tracks those `.VERSION`/`SCHEMAS.VERSION` files plus the `mise.toml` `[tools]` pins (Dependabot still owns Python/uv, Actions, Docker, docker-compose — disjoint scopes; its *version* updates cover only deps declared in `pyproject.toml`, while indirect deps ride Dependabot **security** updates, a repo-settings toggle — see [`docs/operations/maintenance.md` §2](docs/operations/maintenance.md)). On a new upstream release Renovate bumps the pinned version and, on a `.VERSION` bump, runs `dev/update_vendored_assets.py` (`mise run update-vendor`) as a tightly allow-listed postUpgradeTask to re-download, rehash, and re-apply the OSV `ECL-` prefix patch. Auto-merge policy: the JSON schemas are payload-validated by `publication/tests` (+ the OSV ecosystem drift guard in `publication/tests/test_osv.py` + `publication/tests/test_schemas_valid.py`), so their PRs auto-merge on green CI; the frontend assets have no behavioral tests, so their PRs are review-only (manual smoke); the mise-toolchain PRs are grouped and review-only (git-cliff/trivy only execute at release time, so green CI can't vouch for them). Schemas are tag-pinned (`osv-schema`/`cve-schema` tags; OSV carries the local `ECL-` patch — see `publication/schemas/SCHEMAS.md`).
...
- `admin_console/` — sidebar shell at `/admin/` (Inbox, Projects, Users, Groups, Invitations, CVE Assignment, Publication, GHSA, Audit logs, Access log, Maintenance — review decisions happen on the advisory page, surfaced via the Inbox); views split into `admin_console/views/<section>.py`. The Invitations section lists outstanding `PendingInvitation` rows across all advisories with per-row re-send (refreshes the expiry window) and cancel. The GHSA section (feature-gated on `GHSA_FEATURE_ENABLED`) is a GHSA operations + observability dashboard whose action buttons POST to the `ghsa:` endpoints. Django's built-in admin is **not** mounted — it was removed (no `django.contrib.admin`, no `/django-admin/`) so every mutation goes through the audited service layer.
...
Notable knobs: `OIDC_GROUP_CLAIM`, `OIDC_ADMIN_GROUP`, `OIDC_REQUIRE_EMAIL_VERIFIED` (reject an *absent* `email_verified` claim too — an explicit falsey always blocks account create/link; default `False` for Kanidm; INV-OIDC-6), `STEP_UP_REQUIRED` (re-auth gate before publish), `CSP_REPORT_ONLY` (CSP is enforced by default; set `True` for Report-Only) + `CSP_REPORT_URI`, `READYZ_INCLUDE_PUB_REPO` / `READYZ_INCLUDE_BROKER` (extra `/readyz` probes), `RATELIMIT_ENABLE`, `PMI_ROSTER_SYNC_ENABLED` (+ `ECLIPSE_API_*` OAuth2 creds — security-team roster sync, off by default), `SIMILARITY_CHECK_ENABLED` (+ `SIMILARITY_LLM_*` provider/model/key/base-url — LLM duplicate detection, off by default; enabling sends advisory content to the configured provider, INV-SIM-2).
